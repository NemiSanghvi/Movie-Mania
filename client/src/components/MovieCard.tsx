...
const MovieCard: React.FC<MovieProp> = ({ movie }) => {
  return (
    <div className="bg-white rounded-lg shadow-md p-4 hover:shadow-lg transition-shadow w-full sm:w-60">
      <img
        src={`https://image.tmdb.org/t/p/w400/${movie.poster_path}?api_key=${apiKey}`}
        alt={movie.title}
        className="w-full h-full sm:h-64 object-fill rounded-md mb-4"
      />
